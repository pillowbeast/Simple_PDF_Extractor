...
class Reader:
    ...
    def extract_body(self, dic):
        ...
        for key, value in dic.items():
            ...
            if (value['text'].find('Abstract') != -1 or value['text'].find('ABSTRACT') != -1) or (value['text'].find('Introduction') != -1 or value['text'].find('INTRODUCTION') != -1):
                abstract = True
            if abstract == False:
                flag_words.append(value['text'].replace('\n', '').strip())

            if (abstract == True):
                if (value['text'].find('References') != -1):
                    break
                if (len(value['text']) > 7 and value['text'][0].isdigit() == False and value['text'][1].isdigit() == False and value['text'] not in flag_words):
                    body.append(value['text'].replace('\n', ' ').strip())
    ...
    def read_file(self, file_name):
        ...
        try:
            ...
            # pdf_extraced = self.extraction(self.publisher, pdf, f)
            body = self.extract_body(pdf)
            #print(body)
            return body
